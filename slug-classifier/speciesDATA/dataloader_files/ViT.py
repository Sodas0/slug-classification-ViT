...
class SlugDataset(Dataset):
    ...
    def save_dataset_info(self, output_dir='./dataset_info'):
        """
        Save dataset information to files, including class mappings.
        
        Args:
            output_dir (str): Directory to save the dataset information
        ...
        information = self.get_dataset_information()
        
        # Save class mapping (idx to class name)
        idx_to_class = {str(idx): class_name for class_name, idx in information['class_to_idx'].items()}
        class_mapping_path = os.path.join(output_dir, 'class_mapping.json')
        with open(class_mapping_path, 'w') as f:
            json.dump(idx_to_class, f, indent=4)
        
        # Save inverse mapping (class name to idx)
        inverse_mapping_path = os.path.join(output_dir, 'class_to_idx.json')
        with open(inverse_mapping_path, 'w') as f:
            # Convert integer keys to strings for JSON serialization
            class_to_idx_serializable = {k: v for k, v in information['class_to_idx'].items()}
            json.dump(class_to_idx_serializable, f, indent=4)
        
        # Save class list
        classes_path = os.path.join(output_dir, 'classes.json')
        with open(classes_path, 'w') as f:
            json.dump(information['classes'], f, indent=4)
        
        # Save full dataset information
        # Remove any non-serializable items from the information dictionary
        serializable_info = {
            'total_samples': information['total_samples'],
            'positive_samples': information['positive_samples'],
            'negative_samples': information['negative_samples'],
            'num_classes': information['num_classes'],
            'classes': information['classes'],
            'class_to_idx': information['class_to_idx'],
            'transform_type': str(information['transform_type']),
            'class_distribution': information['class_distribution']
        }
        
        # Save all serializable information
        full_info_path = os.path.join(output_dir, 'dataset_info.json')
        with open(full_info_path, 'w') as f:
            json.dump(serializable_info, f, indent=4)
        
        # Create a summary text file with key information
        summary_path = os.path.join(output_dir, 'dataset_summary.txt')
        with open(summary_path, 'w') as f:
            f.write(f"Dataset Summary:\n")
            f.write(f"  Total samples: {information['total_samples']}\n")
            f.write(f"  Slug samples: {information['positive_samples']}\n")
            f.write(f"  Non-slug samples: {information['negative_samples']}\n")
            f.write(f"  Number of classes: {information['num_classes']}\n\n")
            
            f.write(f"Class distribution (all classes):\n")
            for cls, count in sorted(information['class_distribution'].items(), key=lambda x: x[1], reverse=True):
                percentage = (count / information['total_samples']) * 100
                f.write(f"  {cls}: {count} samples ({percentage:.1f}%)\n")
        
        print(f"Dataset information saved to {output_dir}:")
        print(f"  - Class mapping: {class_mapping_path}")
        print(f"  - Class to index mapping: {inverse_mapping_path}")
        print(f"  - Classes list: {classes_path}")
        print(f"  - Full dataset info: {full_info_path}")
        print(f"  - Dataset summary: {summary_path}")
        
        return {
            'class_mapping': class_mapping_path,
            'class_to_idx': inverse_mapping_path,
            'classes': classes_path,
            'dataset_info': full_info_path,
